...
public class QueryCarFilterStorage {
    ...
    public Optional<QueryCarFilter> findById(int id) {
        Optional<QueryCarFilter> result = Optional.empty();
        try(Session session = factory.openSession()) {
            QueryCarFilter queryCarFilter = session.get(QueryCarFilter.class, id);
            if (queryCarFilter != null) {
                result = Optional.of(queryCarFilter);
            }
        }
        return result;
    }
